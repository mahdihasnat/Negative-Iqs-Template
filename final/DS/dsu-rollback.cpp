...
struct dsu_with_rollbacks {
    ...
    dsu_with_rollbacks() {}
    dsu_with_rollbacks(int n) { p.resize(n); rnk.resize(n);
    ...
    void rollback() { if (op.empty()) return;
        ...
        rnk[x.v] = x.rnkv; p[x.u] = x.u; rnk[x.u] = x.rnku;
    }
};
struct query {
    int v, u; bool united;
...
struct QueryTree {
    vector<vector<query>> t; dsu_with_rollbacks dsu; int T;
    QueryTree() {}
    QueryTree(int _T, int n) : T(_T) {
        dsu = dsu_with_rollbacks(n); t.resize(4 * T + 4); }
